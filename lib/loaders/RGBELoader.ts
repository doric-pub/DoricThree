import { BridgeContext, loge, Resource, resourceLoader } from "doric";
import {
  ClampToEdgeWrapping,
  DataTexture,
  DataUtils,
...
function rgbe_error(rgbe_error_code: number, msg?: string) {
  switch (rgbe_error_code) {
    case rgbe_read_error:
      loge("THREE.RGBELoader Read Error: " + (msg || ""));
      break;
    case rgbe_write_error:
      loge("THREE.RGBELoader Write Error: " + (msg || ""));
      break;
    case rgbe_format_error:
      loge("THREE.RGBELoader Bad File Format: " + (msg || ""));
      break;
    default:
    case rgbe_memory_error:
      loge("THREE.RGBELoader: Error: " + (msg || ""));
  }
  return RGBE_RETURN_FAILURE;
}

function fgets(buffer: Buffer, lineLimit?: number, consume?: boolean) {
  const chunkSize = 128;
...
function RGBE_ReadHeader(buffer: Buffer) {
...
export class RGBELoader {
  ...
  async load(context: BridgeContext, resource: Resource) {
    const buffer = await resourceLoader(context).load(resource);
    const byteArray = new Buffer(buffer);
    byteArray.pos = 0;
    const rgbe_header_info = RGBE_ReadHeader(byteArray);

    if (
      RGBE_RETURN_FAILURE !== rgbe_header_info &&
      ...
      if (
        RGBE_RETURN_FAILURE !== image_rgba_data &&
